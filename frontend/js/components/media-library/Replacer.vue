<template>
  <div class="uploader">
    <div><strong>Replace current file</strong></div>
  ...
  export default {
    ...
    methods: {
        ...
        this._uploader = this.uploaderConfig.endpointType === 's3'
          ? new FineUploaderS3({
            options: {
              ...
              request: {
                endpoint: this.uploaderConfig.replaceEndpoint + `?id=${this.media.id}`,
      ...
      loadingFinished: function (loadingMedia, savedMedia) {
        ...
        this.$store.commit(MEDIA_LIBRARY.DONE_UPLOAD_MEDIA, loadingMedia) //eslint-disable-line
      },
